...
export async function middleware(request: NextRequest) {
  ...
  const maybeCountryCode = pathSegments[0]?.toLowerCase()

  // Strip stale/invalid country prefixes (e.g. /dk/... when only /id/ exists)
  if (
    maybeCountryCode &&
    maybeCountryCode.length === 2 &&
    !regionMap.has(maybeCountryCode)
  ) {
    const rest = pathSegments.slice(1).join("/")
    redirectPath = rest ? `/${rest}` : ""
  }
